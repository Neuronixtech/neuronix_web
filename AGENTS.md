...
- **Auth is a stub** — `AuthProvider` exists but backend is not built. `api-client.js` reads `VITE_API_BASE_URL` (default `/api`) and `access_token` from localStorage. Contact form uses `setTimeout` placeholder.
